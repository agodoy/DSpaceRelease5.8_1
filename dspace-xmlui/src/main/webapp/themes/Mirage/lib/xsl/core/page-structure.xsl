...
<!-- Main structure of the page, determines where header, footer, body, navigation 
	...
	Author: art.lowel at atmire.com Author: lieven.droogmans at atmire.com Author: 
	ben at atmire.com Author: Alexey Maslov -->
...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<div id="ds-main">
						...
						<!--The trail is built by applying a template over pageMeta's trail 
							children. -->
	...
	<xsl:template name="buildHeader">
		<div id="ds-header-wrapper">
			<div id="ds-header" class="clearfix">
				...
				<!-- escire comentado por peticion de cesar <xsl:choose> <xsl:when test="/dri:document/dri:meta/dri:userMeta/@authenticated 
					= 'yes'"> <div id="ds-user-box"> <p> <a> <xsl:attribute name="href"> <xsl:value-of 
					select="/dri:document/dri:meta/dri:userMeta/ dri:metadata[@element='identifier' 
					and @qualifier='url']"/> </xsl:attribute> <i18n:text>xmlui.dri2xhtml.structural.profile</i18n:text> 
					<xsl:value-of select="/dri:document/dri:meta/dri:userMeta/ dri:metadata[@element='identifier' 
					and @qualifier='firstName']"/> <xsl:text> </xsl:text> <xsl:value-of select="/dri:document/dri:meta/dri:userMeta/ 
					dri:metadata[@element='identifier' and @qualifier='lastName']"/> </a> <xsl:text> 
					| </xsl:text> <a> <xsl:attribute name="href"> <xsl:value-of select="/dri:document/dri:meta/dri:userMeta/ 
					dri:metadata[@element='identifier' and @qualifier='logoutURL']"/> </xsl:attribute> 
					<i18n:text>xmlui.dri2xhtml.structural.logout</i18n:text> </a> </p> </div> 
					</xsl:when> <xsl:otherwise> <div id="ds-user-box"> <p> <a> <xsl:attribute 
					name="href"> <xsl:value-of select="/dri:document/dri:meta/dri:userMeta/ dri:metadata[@element='identifier' 
					and @qualifier='loginURL']"/> </xsl:attribute> <i18n:text>xmlui.dri2xhtml.structural.login</i18n:text> 
					</a> </p> </div> </xsl:otherwise> </xsl:choose> -->
			</div>
		</div>
	</xsl:template>
	...
	<xsl:template name="buildTrail">
		<div id="ds-trail-menu">
			<div id="ds-trail-menu-int" align="center">
				<a>
					<xsl:attribute name="href">
                       <xsl:value-of
						select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]" />
				...
				|
				<xsl:text>  </xsl:text>
				<a href="http://www.biiacs.cide.edu">
					Home BIIACS
				</a>
				<xsl:text>  </xsl:text>
				|
				<xsl:text>  </xsl:text>
				<a href="http://www.cide.edu">
					Home CIDE
				</a>
				<xsl:text>  </xsl:text>
				|
			...
			<div about="{$handleUri}">
				...
				<a rel="license" href="{$ccLicenseUri}" alt="{$ccLicenseName}"
					...
					<img>
						<xsl:attribute name="src">
                        <xsl:value-of
							select="concat($theme-path,'/images/cc-ship.gif')" />
                     </xsl:attribute>
	...
	<!-- Like the header, the footer contains various miscellanious text, links, 
				...
				<div id="ds-footer-center">
					<div style="width=100%">
						<p style="text-align: center; ">
							Banco de Informaci&#xF3;n para la Investigaci&#xF3;n Aplicada en
							Ciencias Sociales
							<br></br>
							...
							M&#xE9;xico, D.F.
							<br></br>
							Conmutador: 5727-9800 ext. 2417
							<br></br>
							Copyright 2008 www.biiacs.cide.edu All rights reserved
						</p>
					</div>
				</div>
			</div>
	...
	<!-- The meta, body, options elements; the three top-level elements in the 
		schema -->
					...
					<xsl:when test="starts-with($request-uri, 'page/about')">
						<div>
							...
							<p>To add your own content to this page, edit
								...
								will need to create an additional xsl:when block and match the
								request-uri to whatever page
								you are adding. Currently, static pages created through altering
								XSL are only available
