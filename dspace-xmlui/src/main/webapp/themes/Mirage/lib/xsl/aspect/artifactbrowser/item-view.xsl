...
<!-- Rendering of a list of items (e.g. in a search or browse results page) 
...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
	...
	<!--these templates are modfied to support the 2 different item list views 
		that can be configured with the property 'xmlui.theme.mirage.item-list.emphasis' 
		in dspace.cfg -->
	...
	<xsl:template name="itemSummaryList-DIM">
		...
		<xsl:variable name="emphasis"
			select="confman:getProperty('xmlui.theme.mirage.item-list.emphasis')" />
								...
								<xsl:for-each
									...
									<span>
										<a>
											<xsl:attribute name="href">
                                                 <xsl:value-of
												select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]" />/dspace/browse?value=<xsl:copy-of
												select="node()" />&amp;type=author
                                            </xsl:attribute>
											<xsl:copy-of select="node()" />
										</a>
									</span>
