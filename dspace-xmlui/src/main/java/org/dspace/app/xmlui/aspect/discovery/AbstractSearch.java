...
public abstract class AbstractSearch extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
	...
	private static final Message T_sort_by_relevance = message(
			"xmlui.ArtifactBrowser.AbstractSearch.sort_by.relevance");
	 ...
	 * <p/>
	 * This validity object should never "over cache" because it will perform
	 * the search, and serialize the results using the DSpaceValidity object.
	 */
	...
	protected void buildSearchResultsDivision(Division search)
		...
		if (searchScope instanceof Community) {
			...
			org.dspace.app.xmlui.wing.element.List titulos = results.addList("titulos", "simple", "rendertitulos");
			titulos.addItem("Fecha", "renderFecha").addContent(message("fecha"));
			titulos.addItem("Titulo", "renderTitulo").addContent(message("titulo"));
			titulos.addItem("Autor", "renderAutor").addContent(message("autor"));
		...
		if (queryResults != null && 0 < queryResults.getDspaceObjects().size()) {
			...
			int pagesTotal = (int) ((this.queryResults.getTotalSearchResults() - 1) / this.queryResults.getMaxResults())
					+ 1;
			...
			for (DSpaceObject resultDso : queryResults.getDspaceObjects()) {
				if (resultDso instanceof Community || resultDso instanceof Collection) {
					if (referenceSet == null) {
						...
						// Set a heading showing that we will be listing
						// containers that matched:
						referenceSet.setHead(T_head2);
					}
					if (resultDso != null) {
						org.dspace.app.xmlui.wing.element.List titulos = search.addList("titulos", "simple",
								"rendertitulos");
						titulos.addItem("Fecha", "renderFecha").addContent(message("fecha"));
						titulos.addItem("Titulo", "renderTitulo").addContent(message("titulo"));
						titulos.addItem("Autor", "renderAutor").addContent(message("autor"));
			...
			referenceSet = results.addReferenceSet("search-results-repository", ReferenceSet.TYPE_SUMMARY_LIST, null,
	...
	public void performSearch(DSpaceObject scope) throws UIException, SearchServiceException {
		...
		if (groupBy != null && !groupBy.equalsIgnoreCase("none")) {
			...
			// TODO: This is a hack to get Publications (Articles) to always be
			// at the top of Groups.
		...
		// The fields to use for similarity. NOTE: if possible, these should
		// have a stored TermVector
		...
		// mlt.maxntp
		// maximum number of tokens to parse in each example doc field that is
		// not stored with TermVector support.

		// mlt.boost
		// [true/false] set if the query will be boosted by the interesting term
		// relevance.
		...
		/*
		 * ClientUtils.escapeQueryChars(location) //f.category.facet.limit=5
		 * 
		 * for(Enumeration en = request.getParameterNames();
		 * en.hasMoreElements();) { String key = (String)en.nextElement();
		 * if(key.endsWith(".facet.limit")) { filePost.addParameter(key,
	 ...
	 * user.
	 * <p/>
	 * The search scope when preformed by url, i.e. they are at the url
	 * handle/xxxx/xx/search then it is fixed. However at the global level the
	 * search is variable.
	 *
	...
	protected void buildSearchControls(Division div) throws WingException, SQLException {
		...
		org.dspace.app.xmlui.wing.element.List controlsList = div.addList("search-controls",
		...
		/*
		 * Cell groupCell = controlsRow.addCell(); try { // Create a drop down
		 * of the different sort columns available
		 * groupCell.addContent(T_group_by); Select groupSelect =
		 * groupCell.addSelect("group_by"); groupSelect.addOption(false, "none",
		 * T_group_by_none);
		 * 
		 * 
		 * String[] groups = {"publication_grp"}; for (String group : groups) {
		 ...
		 * WingException("Unable to get group options", se); }
		 */
		...
		if (searchSortConfiguration != null) {
			for (DiscoverySortFieldConfiguration sortFieldConfiguration : searchSortConfiguration.getSortFields()) {
				...
				sortSelect.addOption(
						(sortField.equals(currentSort)
								|| sortFieldConfiguration.equals(searchSortConfiguration.getDefaultSort())),
						sortField, message("xmlui.ArtifactBrowser.AbstractSearch.sort_by." + sortField));
			}
		}
		...
		orderSelect.addOption(SortOption.DESCENDING.equalsIgnoreCase(parameterOrder), SortOption.DESCENDING,
				T_order_desc);
	...
	protected void logSearch() {
		...
		/**
		 * TODO: Maybe we can create a default "type" facet for this will give
		 * results for Items, Communities and Collection types benefits... no
		 * iteration over results at all to sum types leaves it upto solr...
		 * 
		 * for (Object type : queryResults.getHitTypes()) { if (type instanceof
		 * Integer) { switch (((Integer)type).intValue()) { case Constants.ITEM:
		 * countItems++; break; case Constants.COLLECTION: countCollections++;
		 * break; case Constants.COMMUNITY: countCommunities++; break; } } }
		 */
		String logInfo = "";
		...
		try {
			...
			// Ignore, as we are only trying to get the scope to add detail to
			// the log message
		}

		log.info(LogManager.getHeader(context, "search",
				logInfo + "query=\"" + (queryArgs == null ? "" : queryArgs.getQuery()) + "\",results=("
						+ countCommunities + "," + countCollections + "," + countItems + ")"));
	}
}
